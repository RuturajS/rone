...
You can run ROne as a lightweight container. See [DOCKER.md](DOCKER.md) for full instructions, management commands, and Docker Compose configuration.

### Quick Start with Docker
```bash
docker build -t rone:latest .
docker run -d --name rone -v $(pwd)/config.yaml:/app/config.yaml rone:latest
```
